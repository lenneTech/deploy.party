...
<template>
  <div class="w-full min-h-screen flex flex-col justify-center items-center">
    <div class="w-full flex flex-col items-center mb-20">
      <h1
        class="text-[12rem] font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-primary-600 to-primary-400"
      >
        Oops!
      </h1>
      <h2 v-if="error?.statusCode" class="text-3xl max-w-5xl text-center text-foreground">
    ...
    <pre
      v-if="debugWord === 'debug'"
      class="w-full max-w-3xl mb-5 mx-auto bg-black/80 text-white font-mono p-2 rounded-lg overflow-x-auto"
    >
      {{ { error } }}
    </pre>
  </div>
