...
const { controlledValues, meta, validate } = useForm({
...
async function submit() {
  ...
  const { data } = await useUpdateProjectMutation(
    {
  ...
  const { data } = await useUpdateProjectMutation(
    {
      ...
      input: {
        subscribers:
          project.value?.subscribers?.map((user) => user.id).filter((id) => id !== user.value.id) ?? ([] as string[]),
      },
    },
    ['id'],
  );
        ...
        <div class="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10 sm:pl-16">
          ...
          >
            <div v-show="props.context.showInner" ref="target" class="pointer-events-auto w-screen max-w-md">
              <form
                novalidate
                class="flex h-full flex-col divide-y divide-border bg-background shadow-xl"
                @submit.prevent="null"
              >
                  ...
                  <div class="flex flex-1 flex-col justify-between">
                    <div class="divide-y divide-gray-200 px-4 sm:px-6">
                      <div class="space-y-6 pb-5 pt-6">
                        ...
                        <FormInput type="text" label="Identifier" name="identifier" />
                        <BaseButton v-if="!isSubscribed" type="button" @click="subscribeToProject">
                          Inform me about changes
                        </BaseButton>
                        <BaseButton v-else type="button" @click="unsubscribeToProject"> Unsubscribe</BaseButton>
                      </div>
                    </div>
                  </div>
                </div>
